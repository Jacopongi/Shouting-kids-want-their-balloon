function [KidArray, BalloonArray] = Flocking(KidArray, BalloonArray, Room, FlockingFlag, VideoFlag)
% Summary: simulate flocking behavior in reaching targets.
% Description: robots try to reach their target (specified by an identity number).
% When multiple robots come closer, they start to move together (flocking).
...
% NOTE: FlockingFlag parameter allows user to choose if flocking behavior has to be activated.
% It is possible to run the simulation with just collision avoidance behavior.
% Set FlockingFlag = false, for this second case.
 

% Parameters
num_robots = KidArray.N;            % Number of robots
num_obstacles = BalloonArray.N;     % Number of obstacles

% Simulation set-up
dt = 0.1;             % Time step
% t_end = 20;         % End time 
t_index = 0;
max_iterations = num_robots * 400;
...
% Values for case without flocking:  0      0     5.0     1.5
...
if VideoFlag
    v = VideoWriter('FlockingNC.avi');    % Create a video writer object
...
disp(' ')
disp(['Let''' 's open the playroom!'])
disp(['So we have ' num2str(num_robots) ' kids and ' num2str(num_obstacles) ' balloons. Great!'])
disp('Are you ready for flocking? 3,2,1... Go!')
disp(' ')

% Main simulation loop
% for t_index = 1:floor(t_end/dt)      
while sum(RecognitionArray) ~= min(num_robots,num_obstacles) && t_index < max_iterations
    ...
    % Compute new velocity for each robot
    for i = 1:num_robots
        pos_i = positions(i, :);
        vel_i = velocities(i, :);
    ...
    stp = text(-0.01*Room.Width, -0.04*Room.Height, ['Step: ' num2str(t_index)]);
...
if t_index < max_iterations
    disp([ 'Total number of steps: ' num2str(t_index) ]);
else
    disp('Stop! My patience has limits!')
end
